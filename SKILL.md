...
- **macOS desktop companion:** install or rebuild the transparent menu-bar app at the repository's small `97 px` resting width. Walking left, walking right, and rolling are rendered at `1.5×`, with smooth bottom-anchored panel expansion so the full body stays visible. It uses calm animation timing and a lossless 16-frame desktop atlas (`6144×3328`, `384×416` cells) derived from the eight `720×720` green-screen live-dog clips. The app refreshes at `60 Hz`, uses a short eased adjacent-frame blend, joins actions with endpoint holds and a tail-frame-to-head-frame crossfade, tells an offline cold joke on double-click, responds to back-and-forth pointer petting with the expectant action, eats only at `08:30`, `12:00`, and `19:00` local time for 30 minutes per meal, uses the expectant/approach action while dragged upward and dropped, plays a roughly `6.8 s` rolling/belly-up action about every 5–10 minutes during recent computer use and holds the final belly-up pose for `2.2 s`, performs a fixed walk-left → head-tilt → roll → walk-right routine after three minutes without direct interaction and repeats it after another three ignored minutes, retreats into a screen edge after five ignored minutes when parked in a corner, returns with the expectant action on hover, hides during typing/full-screen/video playback, shows a self-dismissing hourly movement-and-water bubble during active work, and offers a right-click **暂时退出妹妹** action directly on the pet.
...
Preserve the single-instance contract when rebuilding: `LSMultipleInstancesProhibited` must remain enabled, the resting desktop width must remain `97 px`, walking and rolling must use the verified `1.5×` scale, and launch commands must use `open` without `-n`.

Preserve the interaction schedule when rebuilding: upward dragging and the gravity drop use the expectant row, while autonomous rolling/belly-up is governed by one randomized `5–10 minute` timer and only starts during recent computer activity from an idle or review state. A roll plays every source frame once, then holds the terminal belly-up pose for `2.2 s`, for about `6.8 s` total without looping or wrapping to frame 0. Menu and single-click roll triggers remain available and reset that timer.

Preserve the ignored-pet routine when rebuilding: after `180 s` without direct interaction, wait until Meimei is safely visible in idle/review, then play walk-left → head-tilt → roll → walk-right as one state-machine sequence. Each row change must use the normal endpoint hold and `0.62 s` crossfade. Direct interaction cancels the sequence and resets its timer; if she remains ignored, schedule the next sequence `180 s` after the previous one completes. Meals, reminders, dragging, edge hiding, pause, focus protection, and cinema mode take priority.

Preserve the meal contract when rebuilding: the eating row loops only during three local-time windows—`08:30–09:00`, `12:00–12:30`, and `19:00–19:30`. Remove eating from ordinary random play, single-click choices, pointer petting, edge emergence, jumping, and the direct action menu. Petting and edge emergence use the expectant row instead.
...
Preserve the separate desktop action atlas when rebuilding: `assets/desktop-source/video-actions.webp` is a lossless `6144×3328` WebP with `384×416` cells and eight complete 16-frame rows in this order: head tilt, eating, rolling, waiting, startup, walking left, walking right, and expectant. Build it directly from the approved `720×720` green-screen MP4 pixels without another lossy encode, retain the attached food bowl, remove detached floor/keying residue without cutting away paws or feathered fur, and do not redraw or restyle them as an electronic dog. All visible desktop modes and direct drag/edge frames must resolve to rows `11–18`; `illustrated_fallback=false` is a verified invariant. Keep these rows desktop-only so the Codex v2 `8×11` atlas and state semantics remain unchanged.

Preserve action continuity when rebuilding: keep 16 source frames per desktop action, refresh the app at `60 Hz`, and use the alpha-preserving eased adjacent-frame blend only during the last `42%` of each source-frame interval. The blend weights must add to one through plus-lighter compositing so the transparent window never makes the pet pulse in opacity. Non-looping actions must hold frame 0 briefly, play through the terminal frame exactly once, hold that terminal frame, and never modulo-wrap to frame 0 before the mode ends. On every row change, preserve any active subframe blend, hold the incoming action on frame 0 while blending from the outgoing displayed frame for `0.62 s`, then start the incoming animation. Preserve the calm autonomous behavior weights, original per-action durations, and `10–22 s` idle windows.

Preserve direct interactions when rebuilding: single-click uses the normal play selector only after the double-click interval expires; double-click cancels that pending single-click and shows a bundled offline cold joke. Back-and-forth pointer movement over the pet accumulates an `84 pt` petting gesture, plays the expectant action once, and then observes a `12 s` cooldown so ordinary hovering cannot repeatedly restart it.
